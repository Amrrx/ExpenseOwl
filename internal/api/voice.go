...
// ReceiptParseRequest is the HTTP request for receipt image parsing
type ReceiptParseHTTPRequest struct {
...
// ParseReceiptExpense handles receipt image parsing
func (h *Handler) ParseReceiptExpense(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		writeJSON(w, http.StatusMethodNotAllowed, ErrorResponse{Error: "Method not allowed"})
		return
	}
	...
	// Parse request body
	var req ReceiptParseHTTPRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		writeJSON(w, http.StatusBadRequest, ErrorResponse{Error: "Invalid request body"})
		return
	}
	...
	// Decode image data
	imageBytes, err := ai.DecodeBase64Audio(req.ImageData) // Reuse the base64 decoder
	if err != nil {
		writeJSON(w, http.StatusBadRequest, ErrorResponse{Error: fmt.Sprintf("Failed to decode image: %v", err)})
		return
	}
